...
export default async function handler(req, res) {
  ...
  try {
    ...
    const fileName = `${Date.now()}_${file.originalFilename || 'video'}`;
    const { url } = await put(`uploads/video/${fileName}`, arrayBuffer, { access: 'public', contentType });
...
function parseSingleFile(req) {
  return new Promise((resolve, reject) => {
    ...
    form.parse(req, (err, fields, files) => {
      ...
      const f = files.file || files.video || Object.values(files)[0];
