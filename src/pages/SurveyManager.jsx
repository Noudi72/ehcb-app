  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        {/* Aktionen immer sichtbar für mobile Geräte */}
        <div className="flex flex-wrap gap-2 mt-3 sm:mt-0 sm:flex-col sm:space-y-1 min-w-0 flex-shrink-0">
          <button
            onClick={() => onEdit(survey)}
            className={`flex items-center px-3 py-1 text-xs font-medium rounded transition-colors whitespace-nowrap ${
              isDarkMode 
          ...
          >
            📊
            <span className="ml-1">Ergebnisse</span>
          </button>
          ...
          >
            {survey.active ? '⏸️' : '▶️'}
            <span className="ml-1">{survey.active ? 'Pausieren' : 'Aktivieren'}</span>
          </button>
...
export default function SurveyManager() {
  ...
  const handleToggleStatus = async (survey) => {
    try {
      // Use 'active' field instead of 'status' and only send necessary fields
      const newActive = !survey.active;
      console.log('🔄 Toggling survey status:', survey.id, 'to active:', newActive);
