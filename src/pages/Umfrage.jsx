  ...
  const loadTeamFilteredSurveys = async () => {
    try {
      ...
      if (!isCoach && user) {
        ...
        console.log("🔍 User teams (teams):", user.teams);
        console.log("🔍 User teams (team):", user.team);
        
        // Sammel alle Teams des Users (sowohl neue als auch alte Struktur)
        let userTeams = [];
        ...
        if (user.team) {
          userTeams.push(user.team);
        }
        // Duplikate entfernen
        userTeams = [...new Set(userTeams)];
        
        console.log("🎯 Kombinierte User-Teams:", userTeams);
        
        if (userTeams.length > 0) {
          filteredSurveys = activeSurveys.filter(survey => {
            console.log("🔍 Prüfe Umfrage:", survey.title, "target_teams:", survey.target_teams);
            ...
            const hasMatchingTeam = survey.target_teams.some(targetTeam => 
              userTeams.includes(targetTeam)
            );
          ...
          console.log("⚠️ Spieler hat keine Teams zugewiesen - zeige alle Umfragen");
          filteredSurveys = activeSurveys; // Zeige alle Umfragen wenn Spieler keine Teams hat
        }
      ...
      // Lade Fragen von JSON Server über UmfrageContext
      console.log("📊 Available questions from context:", questions.length);
      ...
      const surveysWithQuestions = (Array.isArray(filteredSurveys) ? filteredSurveys : []).map(survey => {
        console.log("🔍 Processing survey:", survey.title, "questions field:", survey.questions);
        
        // Unterstütze verschiedene Fragen-Strukturen
        let qIds = [];
        
        if (Array.isArray(survey.question_ids)) {
          qIds = survey.question_ids;
        } else if (Array.isArray(survey.questions)) {
          // Wenn questions ein Array von Strings (IDs) ist
          if (survey.questions.length > 0 && typeof survey.questions[0] === 'string') {
            qIds = survey.questions;
          } 
          // Wenn questions ein Array von Objekten ist
          else if (survey.questions.length > 0 && typeof survey.questions[0] === 'object') {
            qIds = survey.questions.map(q => q.id);
          }
        }
        
        console.log("🎯 Question IDs to resolve:", qIds);

        const surveyQuestions = qIds
          .map(id => {
            const question = questionById.get(String(id));
            console.log(`🔍 Resolving question ${id}:`, question ? "✅ Found" : "❌ Not found");
            return question;
          })
          .filter(Boolean)
          .map(q => ({
        ...
        console.log(`📝 Survey "${survey.title}" resolved ${surveyQuestions.length} questions`);
